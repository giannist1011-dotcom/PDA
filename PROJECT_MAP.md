...
### admin.py
- GET /admin/ping → admin_ping @49 — Ελαφρύς έλεγχος του admin password για το login…
- GET /admin/overview → admin_overview @57
- GET /admin/shops → admin_list_shops @103
- GET /admin/shops/{uid} → admin_shop_detail @179
- POST /admin/shops/{uid}/profiles/{pid}/reset-pin → admin_reset_profile_pin @216 — Επαναφορά PIN προφίλ από τον διαχειριστή πλατφόρμας: προσωρινό…
- PATCH /admin/shops/{uid} → admin_update_shop @267
- DELETE /admin/shops/{uid} → admin_delete_shop @283
- GET /admin/subscriptions/expiring → admin_expiring_subscriptions @298 — Συνδρομές/δοκιμές που λήγουν στις επόμενες 7 ημέρες (για…
- GET /admin/leads → admin_leads @320
### ai.py
...
- POST /auth/login → login @151
- POST /auth/demo → start_demo @169 — Δημιουργεί δοκιμαστικό λογαριασμό που λήγει σε 3 ώρες…
- POST /admin/demo/cleanup → admin_demo_cleanup @238 — Καθαρισμός ληγμένων demo λογαριασμών — καλείται από cron…
- GET /auth/offline-profiles → offline_profiles @246 — Λίστα προφίλ για τοπική cache της συσκευής (PWA…
- GET /auth/me → me @257
- GET /profiles → list_profiles @286
- POST /profiles → create_profile @295
- PUT /profiles/{pid} → update_profile @315
- DELETE /profiles/{pid} → delete_profile @338
- POST /profile/select → profile_select @355
- POST /profile/change-pin → change_own_pin @396 — Αλλαγή PIN του ενεργού προφίλ — χρησιμοποιείται στην…
- POST /profile/exit → profile_exit @414 — Return a token with profile cleared (used for…
- POST /auth/verify-owner-pin → verify_owner_pin @426
### checklist.py
...
- GET /orders/address-book → address_book @421 — Γνωστές διευθύνσεις πελατών για autocomplete στη φόρμα παράδοσης…
- GET /orders/{oid} → get_order @448
- POST /orders/{oid}/activate → activate_order @458 — Move a scheduled order to active (fired /…
- POST /orders/{oid}/cancel → cancel_order @478
- DELETE /orders/{oid} → delete_order @503
- GET /customers → list_customers @516 — Aggregate customers from phone/delivery orders, grouped by phone
### promo.py
...
- admin-shops/ShopModal.jsx (297 γρ): ShopModal@22
...
- AddressAutocomplete.jsx (211 γρ): AddressAutocomplete@33
...
- OrderPanel.jsx (434 γρ): OrderPanel@40
