...
### orders.py
- GET /orders/next-number → next_order_number @128
- POST /orders → create_order @133
- GET /orders/scheduled → list_scheduled_orders @184
- GET /orders → list_orders @228
- GET /orders/count → count_orders @251 — Συνολικό πλήθος για τα φίλτρα του ιστορικού —…
- GET /orders/live-map → live_map_orders @364 — Παραγγελίες παράδοσης των τελευταίων 30' με συντεταγμένες για…
- POST /orders/live-map/clear → clear_live_map @417 — Χειροκίνητος καθαρισμός: κρύβει από τον χάρτη όλες τις…
- GET /orders/address-book → address_book @425 — Γνωστές διευθύνσεις πελατών για autocomplete στη φόρμα παράδοσης…
- GET /orders/heatmap → orders_heatmap @465 — Heatmap διευθύνσεων παράδοσης για τα Στατιστικά: σημεία (lat/lng)…
- GET /orders/{oid} → get_order @518
- POST /orders/{oid}/activate → activate_order @528 — Move a scheduled order to active (fired /…
- POST /orders/{oid}/cancel → cancel_order @548
- DELETE /orders/{oid} → delete_order @575
- GET /customers → list_customers @590 — Aggregate customers from phone/delivery orders, grouped by phone
### promo.py
...
- history/OrderDetailModal.jsx (222 γρ): OrderDetailModal@32
...
- public-menu/StoreInfoSection.jsx (75 γρ): StoreInfoSection@7
...
- AddressAutocomplete.jsx (341 γρ): AddressAutocomplete@83
...
- OrderPanel.jsx (449 γρ): OrderPanel@40
