...
### fleet.py
- POST /fleet/register → fleet_register @220
- POST /fleet/login → fleet_login @255
- GET /fleet/me → fleet_me @266
- GET /fleet/members → fleet_list_members @272
- POST /fleet/members → fleet_create_member @280
- PUT /fleet/members/{mid} → fleet_update_member @296
- DELETE /fleet/members/{mid} → fleet_delete_member @316
- POST /fleet/member/select → fleet_member_select @352
- POST /fleet/member/exit → fleet_member_exit @365
- POST /fleet/code → fleet_code_lookup @371 — Έλεγχος invite code: όνομα εταιρείας + λίστα οδηγών…
- POST /fleet/join → fleet_join @384 — Νέος οδηγός: invite code + όνομα + PIN…
- POST /fleet/code/select → fleet_code_select @413 — Επανασύνδεση οδηγού από το κινητό: invite code +…
- POST /fleet/orders → fleet_create_order @429
- GET /fleet/board → fleet_board @455 — Ο πίνακας του συντονιστή: παραγγελίες ημέρας + feed…
- GET /fleet/driver/board → fleet_driver_board @474 — Η οθόνη του οδηγού: ελεύθερες + δικές του…
- POST /fleet/orders/{oid}/claim → fleet_claim_order @492 — ΑΤΟΜΙΚΟ claim: μόνο ένας οδηγός παίρνει την παραγγελία…
- POST /fleet/orders/{oid}/status → fleet_order_status @516
- POST /fleet/orders/{oid}/assign → fleet_assign_order @543
- POST /fleet/orders/{oid}/cancel → fleet_cancel_order @574
- GET /fleet/pickup-names → fleet_pickup_names @589 — Ονόματα καταστημάτων παραλαβής που έχουν ξαναχρησιμοποιηθεί (autocomplete).
- GET /fleet/address-book → fleet_address_book @596 — Πρόσφατες διευθύνσεις της ομάδας για το AddressAutocomplete (μορφή…
- GET /fleet/day-summary → fleet_day_summary @614 — Απλά σύνολα ημέρας ανά οδηγό: παραδόσεις + μετρητά…
### menu.py
...
- FleetJoin.jsx (209 γρ): FleetJoin@15
- FleetLogin.jsx (122 γρ): FleetLogin@9
- FleetMembers.jsx (218 γρ): FleetMembers@20
- FleetRegister.jsx (172 γρ): Field@10, FleetRegister@21
- FleetSelect.jsx (132 γρ): FleetSelect@11
