...
### auth.py
- POST /auth/register → register @68
- POST /auth/login → login @152
- POST /auth/demo → start_demo @170 — Δημιουργεί δοκιμαστικό λογαριασμό που λήγει σε 3 ώρες…
- POST /admin/demo/cleanup → admin_demo_cleanup @240 — Καθαρισμός ληγμένων demo λογαριασμών — καλείται από cron…
- GET /auth/offline-profiles → offline_profiles @248 — Λίστα προφίλ για τοπική cache της συσκευής (PWA…
- GET /auth/me → me @259
- GET /profiles → list_profiles @296
- POST /profiles → create_profile @305
- PUT /profiles/{pid} → update_profile @326
- DELETE /profiles/{pid} → delete_profile @352
- POST /profile/select → profile_select @369
- POST /profile/change-pin → change_own_pin @410 — Αλλαγή PIN του ενεργού προφίλ — χρησιμοποιείται στην…
- POST /profile/exit → profile_exit @428 — Return a token with profile cleared (used for…
- POST /auth/verify-owner-pin → verify_owner_pin @440
### billing.py
...
### fleet.py
- POST /fleet/register → fleet_register @266
- POST /fleet/login → fleet_login @301
- GET /fleet/me → fleet_me @312
- GET /fleet/members → fleet_list_members @318
- POST /fleet/members → fleet_create_member @326
- POST /fleet/members/{mid}/reset-password → fleet_reset_member_password @378 — Νέος προσωρινός κωδικός διανομέα — υποχρεωτική αλλαγή στην…
- PUT /fleet/members/{mid} → fleet_update_member @394
- DELETE /fleet/members/{mid} → fleet_delete_member @414
- POST /fleet/member/select → fleet_member_select @450
- POST /fleet/member/exit → fleet_member_exit @463
- POST /fleet/driver/login → fleet_driver_login @490 — Είσοδος διανομέα με τηλέφωνο/email + κωδικό (προσωρινό ή…
- POST /fleet/driver/change-password → fleet_driver_change_password @506
- POST /fleet/driver/select → fleet_driver_select @517 — Επιλογή εταιρείας → πλήρες fleet token (team +…
- POST /fleet/orders → fleet_create_order @536
- GET /fleet/board → fleet_board @562 — Ο πίνακας του συντονιστή: παραγγελίες ημέρας + feed…
- GET /fleet/driver/board → fleet_driver_board @581 — Η οθόνη του οδηγού: ελεύθερες + δικές του…
- POST /fleet/orders/{oid}/claim → fleet_claim_order @599 — ΑΤΟΜΙΚΟ claim: μόνο ένας οδηγός παίρνει την παραγγελία…
- POST /fleet/orders/{oid}/status → fleet_order_status @623
- POST /fleet/orders/{oid}/assign → fleet_assign_order @650
- POST /fleet/orders/{oid}/cancel → fleet_cancel_order @681
- GET /fleet/pickup-names → fleet_pickup_names @696 — Ονόματα καταστημάτων παραλαβής που έχουν ξαναχρησιμοποιηθεί (autocomplete).
- GET /fleet/address-book → fleet_address_book @703 — Πρόσφατες διευθύνσεις της ομάδας για το AddressAutocomplete (μορφή…
- GET /fleet/day-summary → fleet_day_summary @721 — Απλά σύνολα ημέρας ανά οδηγό: παραδόσεις + μετρητά…
### menu.py
...
- users: core.py, routers/admin.py, routers/auth.py, routers/billing.py, routers/menu.py, routers/onboarding.py, routers/orders.py, routers/promo.py, routers/public_menu.py, routers/tables.py, server.py
...
- AppShell.jsx (506 γρ): BetaBadge@88, DemoBanner@112, AppShell@155
...
- ProtectedRoute.jsx (33 γρ): ProtectedRoute@12
