...
- POST /fleet/orders → fleet_create_order @741
- GET /fleet/board → fleet_board @768 — Ο πίνακας του συντονιστή: παραγγελίες ημέρας + feed…
- GET /fleet/driver/board → fleet_driver_board @790 — Η οθόνη του οδηγού: ελεύθερες + δικές του…
- POST /fleet/orders/{oid}/claim → fleet_claim_order @819 — ΑΤΟΜΙΚΟ claim: μόνο ένας οδηγός παίρνει την παραγγελία…
- POST /fleet/orders/{oid}/status → fleet_order_status @843
- POST /fleet/orders/{oid}/assign → fleet_assign_order @871
- POST /fleet/orders/{oid}/cancel → fleet_cancel_order @904
- PUT /fleet/orders/{oid} → fleet_edit_order @918 — Επεξεργασία παραγγελίας από τον συντονιστή. Πριν το claim…
- POST /fleet/orders/{oid}/urgent → fleet_set_urgent @946 — «⚡ Επείγον»: καρφιτσώνει την παραγγελία πρώτη στις «Ελεύθερες»…
- POST /fleet/orders/{oid}/problem → fleet_report_problem @965 — Ο οδηγός σημαίνει πρόβλημα σε claimed παραγγελία του…
- POST /fleet/orders/{oid}/problem/resolve → fleet_resolve_problem @994
- POST /fleet/driver/shift → fleet_driver_shift @1009 — «Ξεκινάω/Τέλος βάρδιας»: on_shift στο μέλος (πράσινη κουκκίδα στον…
- GET /fleet/driver/stats → fleet_driver_stats @1043 — Στατιστικά του ίδιου του οδηγού: σύνολο παραδόσεων, σήμερα/εβδομάδα,
- GET /fleet/driver/orders → fleet_driver_orders @1102 — Ιστορικό παραγγελιών του οδηγού (claimed/παραδομένες) με φίλτρο ημέρας…
- GET /fleet/pickup-names → fleet_pickup_names @1135 — Ονόματα καταστημάτων παραλαβής που έχουν ξαναχρησιμοποιηθεί (autocomplete).
- GET /fleet/address-book → fleet_address_book @1142 — Πρόσφατες διευθύνσεις της ομάδας για το AddressAutocomplete (μορφή…
- GET /fleet/day-summary → fleet_day_summary @1160 — Πλήθη παραγγελιών ανά οδηγό για ημέρα ή εύρος…
### menu.py
...
- fleet/FleetShell.jsx (218 γρ): FleetShell@20
