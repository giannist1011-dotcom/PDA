...
async def on_startup():
    ...
    # Περιοχή «Προγραμματισμένες»: εκκρεμείς + όσες ενεργοποιήθηκαν πρόσφατα
    await db.orders.create_index([("user_id", 1), ("status", 1), ("scheduled_at", 1)])
    ...
    await db.fleet_accounts.create_index("email", unique=True, sparse=True)
    ...
    # Admin dashboard (Επισκόπηση): παραγγελίες σήμερα/30ήμερο σε όλη την πλατφόρμα
    await db.orders.create_index([("created_at", -1)])
    await db.fleet_orders.create_index([("created_at", -1)])
    # Χάρτης επέκτασης: συντεταγμένες ανά πόλη — ΜΙΑ γεωκωδικοποίηση ανά πόλη
    await db.admin_city_geo.create_index("key", unique=True)
    ...
    # Print Bridge: poll pending jobs ανά μαγαζί + εύρεση μαγαζιού από bridge token
    await db.print_jobs.create_index([("user_id", 1), ("status", 1), ("created_at", 1)])
