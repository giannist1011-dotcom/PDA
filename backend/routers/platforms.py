...
from core import db, require_staff, require_owner, require_feature, actor_name
from platform_sounds import platform_sound_wav
...
from routers.orders import compute_next_order_number
...
async def _create_pos_order(user: dict, po: dict, due_at: str) -> dict:
    ...
    number = await compute_next_order_number(user)
    doc = {
        "id": str(uuid.uuid4()),
        "user_id": user["id"],
        "order_number": number,
        ...
        "note": po.get("note") or None,
        "delivery_fee": po.get("delivery_fee") or None,
        "discount": None,
        "delivery": {
        ...
        "edits": [],
        # Σήμανση πλατφόρμας — τυπώνεται ως banner στην απόδειξη και δείχνει
        # το countdown στη λίστα παραγγελιών
...
async def change_ready_time(
    ...
    if po.get("order_id"):
        await db.orders.update_one(
            {"id": po["order_id"], "user_id": user["id"]},
            {"$set": {"platform_due_at": fields["due_at"]}},
        )
    return _clean({**po, **fields})
...
async def out_for_delivery(
    ...
    await db.platform_orders.update_one({"id": oid}, {"$set": fields})
    # FleetDeck: αν το κατάστημα έχει ανεβάσει την ίδια παραγγελία σε συνεργαζόμενη
    # εταιρεία διανομής, σημειώνεται κι εκεί ότι έφυγε. Κοινό κλειδί είναι το
    # τηλέφωνο του πελάτη (μόνο τα ψηφία) — δεν αλλάζει κατάσταση/ανάθεση οδηγού.
    phone = "".join(ch for ch in (po.get("customer") or {}).get("phone", "") if ch.isdigit())
    if phone:
        await db.fleet_orders.update_many(
            {
                "store_user_id": user["id"],
                "phone": {"$regex": f"{phone[-10:]}$"},
                "status": {"$in": ["waiting", "scheduled"]},
            },
            {"$set": {"platform_out_for_delivery_at": now, "platform": po["platform"]}},
        )
    return _clean({**po, **fields})
...
async def create_test_order(
    ...
    menu = await db.items.find(
        {"user_id": user["id"], "available": {"$ne": False}},
