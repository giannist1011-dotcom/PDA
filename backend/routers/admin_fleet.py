...
from routers.fleet import add_event, ensure_fleet_team_for_user, next_order_number
...
async def _seed_store_fleet_demo(uid: str, store_name: str, city: str) -> str:
    ...
    if not part:
        ...
        if company:
            team = await db.fleet_teams.find_one(
                {"owner_user_id": company["id"]}, {"_id": 0, "id": 1, "name": 1, "city": 1}
            )
        if not team:
            created = await _create_fleet_company_demo("Demo Διανομές", city)
            team = {"id": created["team_id"], "name": created["team_name"], "city": city}
        part = {
            "id": str(uuid.uuid4()),
            "store_user_id": uid,
...
async def admin_create_demo(body: DemoCreateIn, x_admin_password: Optional[str] = Header(None)):
    ...
    if is_fleet_plan:
        ...
        partner = await _seed_store_fleet_demo(uid, doc["restaurant_name"], doc["store_city"])
    else:
...
async def admin_reset_demo(uid: str, x_admin_password: Optional[str] = Header(None)):
    ...
    if u.get("plan") == "fleet":
        await db.fleet_orders.delete_many({"store_user_id": uid})
        await _seed_store_profiles(uid, now_iso, include_waiter=False)
        partner = await _seed_store_fleet_demo(
