...
from core import db
from routers.admin_admins import get_admin_ctx, scope_city_match
from routers.orders import _nominatim_lookup
...
def _warm_city_geocode(missing: list[str]):
    ...
    async def run():
        for city in missing[:GEOCODE_BATCH]:
            ...
            try:
                lat, lng = await asyncio.to_thread(_nominatim_lookup, city)
...
async def admin_overview(ctx: dict = Depends(get_admin_ctx)):
    ...
    team_owner = {}  # team_id -> owner user_id
    ...
    if owner_ids:
        async for t in db.fleet_teams.find(
            ...
            team_owner[t["id"]] = t["owner_user_id"]
    drivers_by_owner: dict = {}
    if team_owner:
        async for r in db.fleet_members.aggregate([
            {"$match": {"team_id": {"$in": list(team_owner)}, "role": "driver"}},
            {"$group": {"_id": "$team_id", "n": {"$sum": 1}}},
        ]):
            drivers_by_owner[team_owner[r["_id"]]] = r["n"]
    ...
    pos_today = await db.orders.count_documents({**pos_match, "created_at": {"$gte": today_iso}})
    fleet_today = await db.fleet_orders.count_documents({**fleet_match, "created_at": {"$gte": today_iso}})
    pos_30 = await db.orders.count_documents({**pos_match, "created_at": {"$gte": d30}})
    fleet_30 = await db.fleet_orders.count_documents({**fleet_match, "created_at": {"$gte": d30}})
    pos_prev = await db.orders.count_documents({**pos_match, "created_at": {"$gte": d60, "$lt": d30}})
    fleet_prev = await db.fleet_orders.count_documents({**fleet_match, "created_at": {"$gte": d60, "$lt": d30}})
    ...
    pending_parts = await db.fleet_partnerships.find(
    ...
    ).sort("requested_at", -1).to_list(30)
    for p in parts:
        if scoped and not (
