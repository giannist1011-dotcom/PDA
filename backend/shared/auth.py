...
async def register(body: RegisterIn, request: Request):
    ...
    if body.promo_code and body.promo_code.strip():
        claimed = await admin_api.redeem_promo(body.promo_code)
        promo = {
            "code": claimed["code"],
            "type": claimed["type"],
            "value": claimed["value"],
        ...
        billing_note = f"Κωδικός {claimed['code']}: {admin_api.promo_description(claimed)}"
    doc = {
        "id": uid,
        "email": email,
    ...
    if body.plan != "fleet":
        ...
        await pos_api.seed_account(uid, preset, body.has_tables)
...
async def start_demo(body: DemoIn, request: Request):
    ...
    # Lead capture — το κρατά το admin domain (επιβιώνει της διαγραφής του demo)
    await admin_api.record_demo_lead(
        body.email, body.business_name, body.business_type
    )
